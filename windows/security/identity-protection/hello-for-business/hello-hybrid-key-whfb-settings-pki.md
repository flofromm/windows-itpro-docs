...
>[!NOTE]
>The root and all issuing CA certificates must be present in the NTAuth store. By default, Microsoft Enterprise CAs are added to the NTAuth store. If you are using a 3rd party CA, this may not be done by default. If the root or issuing CA certificates for Domain Controller certificates are missing in the NTAuth store, user authentication will fail. 
